...
os.system("snakemake  -p --latency-wait 36000  --jobs 10 -s workflow/quality_control.rules 2>&1 | tee logs/log_quality_control.txt")
end_time = time.time()
...
os.system("snakemake -p --latency-wait 360  --jobs 4 -s workflow/trim.rules 2>&1 | tee logs/log_trim.txt")
...
#os.system("snakemake -p --latency-wait 36000 --cluster \"qsub -l nodes=1:ppn=1,mem=40gb\"  --jobs 10 -s workflow/dea.rules --dag | dot -Tsvg > logs/dea.svg")
tart_time = time.time()
os.system("snakemake -p --latency-wait 36000  --jobs 3 -s workflow/dea.rules 2>&1 | tee logs/log_dea.txt")
end_time = time.time()
