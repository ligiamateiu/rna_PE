...
if(exclude.genes!="NO"){
  ...
  print(paste0("chrY RNA removed:", length(yRNA), "  Genes remaining:", nrow(count.table)))
    ...
    if(length(covariate1)>0 && length(covariate2)>0){
		...
		print("model with two covariates")
		...
		print("model with one covariates")
	} else{
		...
		print("model with no covariates")
	}
...
outtable_normalized_counts<-merge(normalized_counts,gtf_symbols, by=0)
write.table(outtable_normalized_counts, file = paste(outpath, '/library_normalized_gene_counts.tsv', sep = ''),quote=F,col.names=NA,row.names=T,sep="\t")



#     keep <- rowSums(counts(dds.obj)) >= 20
...
generate_allsamples_plots<-function(dds.obj){
    rlog.obj <- rlog(dds.obj, blind=F)
  
  pca_data <- assay(rlog.obj)
...
#plotting samples 
  ...
  sampleDists <- dist(t(assay(rlog.obj)))
  sampleDistMatrix <- as.matrix(sampleDists)
  rownames(sampleDistMatrix) <- paste(rlog.obj$samples.all, rlog.obj$group.all, sep="_")
  colnames(sampleDistMatrix) <- paste(rlog.obj$samples.all, rlog.obj$group.all, sep="_")
...
DEA <- function(control, treat,dds.obj,rlog.obj) 	{
    # export the results
...
#     res.dea$ihw=adj_pvalues(ihw_res)
    ...
    table_logs<-data.frame(assay(rlog.obj),check.names=FALSE)
    table_logs <- merge(table_logs, gtf_symbols, by.x = "row.names", by.y = "gene_id", all.x = TRUE)
	write.table(table_logs,paste(output.path, '/rlog_normalized_gene_counts.tsv',sep=''),quote=F,col.names=TRUE,row.names=FALSE,sep="\t")
...
vsd_de<- data.frame(assay(rlog.obj[rownames(rlog.obj) %in% deg$Row.names,] ),check.names=FALSE)
...
if(plotalldeg=="TOP"){
	...
	normalized_counts <- counts(dds.obj, normalized=TRUE)
	ncount_de_top<-data.frame(normalized_counts[rownames(normalized_counts) %in% topDEG, ,drop=FALSE], check.names = FALSE)
...
#  	p<-ggplot(ncount_de_gene,aes(x=group, y=ncount,fill=group)) + ggtitle(paste(genesymbol,ensid,sep="\t")) +geom_violin(trim=FALSE)+geom_boxplot(width=0.1)+ylab("log (normalized counts)")+theme(panel.background = element_rect(fill = "white",colour = "black",size = 0.5, linetype = "solid"),panel.grid.major = element_line(size = 0.5, linetype = 'solid',colour = "gray"), panel.grid.minor = element_line(size = 0.25, linetype = 'solid',colour = "white"))+theme(axis.text=element_text(size=22),axis.title=element_text(size=22),plot.title = element_text(size = 14))+theme(legend.position = "none")+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  p<-ggplot(ncount_de_gene,aes(x=group, y=ncount,fill=group, label=row.names(ncount_de_gene))) +  ggtitle(paste(genesymbol," ",ensid))+ geom_label_repel(aes(label = rownames(ncount_de_gene),fill = factor(group)), color = 'white',size = 3.5) 
	print(p)
	dev.off()
    }
}
